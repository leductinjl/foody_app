  ...
  const isBookmarked = useSelector(
    (state: RootState) =>
      (state?.bookmarkState?.bookmarks || []).filter(
        (item: { restaurantId: string }) => item?.restaurantId === id
      ).length > 0
  );  
  
  const addBookmark = () =>
    dispatch(BookmarkAction.addBookmark({restaurantId: id}) as any);
  
  const removeBookmark = () =>
    dispatch(BookmarkAction.removeBookmark({restaurantId: id}) as any);
